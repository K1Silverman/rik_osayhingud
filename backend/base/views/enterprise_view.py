...
from ..serializers import FieSerializer, PhysicalSerializer, EnterpriseSerializer
    ...
    def post(self, request):
        ...
        if enterprise_serializer.is_valid():
            ...
            for shareholder_data in shareholders_data:
                shareholder_type = shareholder_data.pop(
    ...
    def patch(self, request, id):
        ...
        if len(shareholders) > 0:
            with transaction.atomic():
                ...
                shareholders_to_remove = current_shareholders.difference(
                    Shareholder.objects.filter(id__in=[s['id'] for s in old_shareholders]))
                ...
                for shareholder in shareholders:
                    ...
                    if shareholder_id:
                        shareholder_obj = get_object_or_404(
                            Shareholder.objects.select_for_update(), pk=shareholder_id)
                        if shareholder.get('nic', None):
                            serializer = PhysicalSerializer(
                                shareholder_obj, data=shareholder, partial=True)
                        ...
                        if shareholder.get('nic', None):
                            ...
                            shareholder_serializer = PhysicalSerializer(
                                data=shareholder, partial=True)
                            ...
                            shareholder['founder'] = False
                            shareholder_serializer = FieSerializer(
                                data=shareholder, partial=True)
                            if shareholder_serializer.is_valid():
                                new_shareholder = shareholder_serializer.save()
                                enterprise.shareholder.add(new_shareholder)
            ...
            serializer = EnterpriseSerializer(
                enterprise, data=request.data, partial=True)
